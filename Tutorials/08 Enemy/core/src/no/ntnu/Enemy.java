...
public class Enemy {
    ...
    private static final float SAFE_BOUNDARY = 10;
    private static final float MIN_X = SAFE_BOUNDARY;
    private static final float MIN_Y = SAFE_BOUNDARY;
    private static final float MAX_X = EnemyExample.SCREEN_WIDTH - SAFE_BOUNDARY;
    private static final float MAX_Y = EnemyExample.SCREEN_HEIGHT - SAFE_BOUNDARY;
    ...
    private void doMovement() {
        ...
        if ((x < SAFE_BOUNDARY && speedX < 0)
                || (x > MAX_X - sprite.getWidth() && speedX > 0)) {
            speedX = -speedX;
        }
        if ((y < SAFE_BOUNDARY && speedY < 0)
